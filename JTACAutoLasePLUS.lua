...
5. Каолицию юнита jtac и противника указывать не нужно. Выбор делается исходя из каолиции юнита jtac
...
Пример:
----------------------------------------------------------------------------------------------------------------------------------
					Имя группы JTAC		Код лазера		Дым				Тип							Цвет						Группа врага     Время
																						vehicle - техника         Green = 0                   или              облучения
																						troop - живая сила      Red = 1                   all - все           в сек.
																						sam - пво                   White = 2										0 - постоянно
																						static - статический	Orange = 3
																						all-все						Blue = 4
JTACAutoLase(       'JTAC1'    ,       1688,         true ,            	 "all" ,                              2 ,                       "all",					30) 
----------------------------------------------------------------------------------------------------------------------------------

! Примечание: Тип - all для статических объектов не работает. Чтобы подсвечивать все статические объекты используйте тип - static а группу - all
...
Пример:
----------------------------------------------------------------------------------------------------------------------------------
             Имя группы JTAC       дистанция (м.) зоны радиосвязи (меню)
InZone       ( 'JTAC1',                    23000)
...
SIDE_COALITION = nil -- каолиция

JTAC_maxDistance = 3500 -- Как далеко JTAC может "видеть" в метрах - 3500 метров по умолчанию
...
-- Поиск текущего Юнита
    ...
    if enemyUnit == nil then
        ...
        if enemyUnit ~= nil then
            ...
            else trigger.action.outSoundForCoalition(SIDE_COALITION, "jtac_visible.ogg") trigger.action.outText(jtacGroupName .. ": нет видимых целей.",10) end
    end
...
function zeroJtac(jtacGroupName)
	   ...
	   if GLOBAL_SCOUT_UNIT~=nil and  GLOBAL_SCOUT_UNIT[jtacGroupName][timer]~=nil then timer.removeFunction(GLOBAL_SCOUT_UNIT[jtacGroupName][timer]);	end
	   GLOBAL_SCOUT_UNIT[jtacGroupName]=nil
end

function StopInZone(jtacGroupName) 
      ...
      for k, v in pairs(GLOBAL_JTAC_UNIT_VISIBLE) do
    	clearMenu(v["group"],jtacGroupName,nil) 
    end	  
end


-- РАЗВЕДКА 
function inScout(jtacGroupName,distance) 
    local x = 1
    local i = 1
	local tempGroup = nil
    local jtacGroup = getGroup(jtacGroupName)
    local jtacUnit = jtacGroup[1]	
    local units = nil
    local groupName = nil
    local nearestUnit = nil
    local jtacPoint = jtacUnit:getPoint()
	...
	if GLOBAL_SCOUT_UNIT[jtacGroupName][timer]==nil then trigger.action.outText(jtacGroupName .. ": поиск целей на дистанции " .. roundNumber (distance,0) .. " м.",5)
                                        for k,v in pairs(GLOBAL_JTAC_UNIT_VISIBLE) do
										  if Group.getByName(v["group"])~=nil then clearLaseMenu(v["group"],jtacGroupName) else 
										  v["group"]=nil 
                                          end
										  end
    ...
    -- Цикл юнитов
                for x = 1, #units do
                        ...
                        if tempDist < distance and land.isVisible(offsetEnemyPos, offsetJTACPos)  then -- проверяем дистанцию
  	...
  	GLOBAL_SCOUT_UNIT[jtacGroupName][timer] =  timer.scheduleFunction(timerScout, {jtacGroupName, distance},timer.getTime() + 2 )  -- Вызов таймера Scout
...
function InZone(jtacGroupName,distance) 
    ...
    -- Цикл юнитов
                for x = 1, #units do
                        ...
                        if tempDist < distance then -- проверяем дистанцию

						tempGroup = Group.getByName(Unit.getGroup(units[x]):getName())

	if GLOBAL_JTAC_UNIT_VISIBLE[groupName] == nil  then laseMenu(tempGroup:getName(), jtacGroupName)  end 
	local _table = GLOBAL_JTAC_UNIT_VISIBLE[groupName]["jtac"][jtacGroupName] 
	if _table == nil  then laseMenu(tempGroup:getName(), jtacGroupName)  end 
	if GLOBAL_JTAC_CURRENT_TARGETS[jtacGroupName]~=nil and GLOBAL_JTAC_UNIT_VISIBLE[groupName] ~= nil and GLOBAL_JTAC_UNIT_VISIBLE[groupName]["jtac"][jtacGroupName]~=nil then tempGroup = Group.getByName(Unit.getGroup(units[x]):getName())  clearLaseMenu(tempGroup:getName(),jtacGroupName)  end 

	                        else 
							 tempGroup = Group.getByName(Unit.getGroup(units[x]):getName())  
						  	 if GLOBAL_JTAC_UNIT_VISIBLE[tempGroup:getName()]~= nil then
							 GLOBAL_JTAC_UNIT_VISIBLE[tempGroup:getName()]["jtac"][jtacGroupName]=nil 
							 clearMenu(tempGroup:getName(),jtacGroupName) 					   
							 end 
		                end 
	     end
		 end
	    ...
	    GLOBAL_JTAC_UNIT_TIMER[jtacGroupName][timer] = timer.scheduleFunction(timerInZone, {jtacGroupName,distance},timer.getTime() + 2 )  -- Вызов таймера InZone
end

-- Очистить меню подсвета
function clearLaseMenu(groupName,jtacGroupName) 
   ...
   missionCommands.removeItemForGroup(gID, {jtacGroupName,"Произвести разведку"} )  
  end


 -- Очистить все меню у группы 
function clearMenu(groupName,jtacGroupName) 
   local gID = Group.getID(Group.getByName(groupName))
   clearLaseMenu(groupName,jtacGroupName) 
...
function laseMenu(groupName, jtacGroupName)
-- Меню для групп в зоне JTAC
   ...
   -- Очистка меню
   clearMenu(groupName,jtacGroupName)  
...
if GLOBAL_JTAC_CURRENT_TARGETS[jtacGroupName]==nil then
   missionCommands.addCommandForGroup(gID, "3 минуты - Все цели", {jtacGroupName,"код лазера 1113"}, JTACAutoLase, jtacGroupName,1113,false,"all",2,"all", 180 ) 
   missionCommands.addCommandForGroup(gID, "3 минуты - ПВО", {jtacGroupName,"код лазера 1113"}, JTACAutoLase, jtacGroupName,1113,false,"sam",2,"all", 180 )  
   missionCommands.addCommandForGroup(gID, "3 минуты - Статические", {jtacGroupName,"код лазера 1113"}, JTACAutoLase, jtacGroupName,1113,false,"static",2,"all", 180 )        
   missionCommands.addCommandForGroup(gID, "3 минуты - Все цели", {jtacGroupName,"код лазера 1688"}, JTACAutoLase, jtacGroupName,1688,false,"all",2,"all", 180 ) 
   missionCommands.addCommandForGroup(gID, "3 минуты - ПВО", {jtacGroupName,"код лазера 1688"}, JTACAutoLase, jtacGroupName,1688,false,"sam",2,"all", 180 )  
   missionCommands.addCommandForGroup(gID, "3 минуты - Статические", {jtacGroupName,"код лазера 1688"}, JTACAutoLase, jtacGroupName,1688,false,"static",2,"all", 180 )        
   missionCommands.addCommandForGroup(gID, "Произвести разведку", {jtacGroupName}, inScout, jtacGroupName , 4500) 
                                                                              end
...
function isSam(unit)
    ...
    local samType = { "stinger", "zsu", "tor", "strela", "s-300", "buk", "kub", "s-125", "osa", "tunguska", "zu", "igla", "patriot", "hawk","vulcan", "gepard", "tow","chaparral","linebacker", "roland", "avenger" }

    for key,value in pairs(samType) do
       if string.match(typeName, value) then
           return true
       end
    end
...
trigger.action.outText('JTACAutoLase PLUS Beta 1.2 by Eagle86', 2);